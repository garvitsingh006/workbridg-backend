...
const newChat = asyncHandler(async (req, res) => {
    ...
    if (type === "individual") {
        if (!otherUserId) throw new ApiError(400, "Other user ID is required for individual chats");
        ...
        if (!project) throw new ApiError(400, "Project ID is required for project chats");
    ...
    const chat = await Chat.initiateChat(participants, type, type === "project" ? project : null);

    res.status(200).json(new ApiResponse(200, chat, "Chat initiated/fetched successfully"));
});
...
const newMessage = asyncHandler(async (req, res) => {
    ...
    res.status(200).json(new ApiResponse(200, updatedChat, "Message sent successfully"));
...
const getAllChats = asyncHandler(async (req, res) => {
    ...
    res.status(200).json(new ApiResponse(200, chats, "Fetched all chats successfully"));
});
...
const markMessagesAsRead = asyncHandler(async (req, res) => {
    ...
    const isParticipant = chat.participants
        .map(p => p.toString())
        .includes(userId.toString());
    if (!isParticipant) throw new ApiError(403, "You are not a participant of this chat");

    // Mark all messages not sent by the user as read
    await chat.markMessagesRead(userId);

    res.status(200).json(new ApiResponse(200, chat, "All messages marked as read"));
})

export { newChat, newMessage, getAllChats, markMessagesAsRead };
